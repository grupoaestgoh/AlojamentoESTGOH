...
*/
  ...
  function inserir_anuncio(Anuncio $anuncio){
    ...
    global $mybd;
		$STH = $mybd->DBH->prepare("Update utilizador Set anu_titulo=:ti,anu_descricao=:de,anu_morada=:mo,anu_email,anu_estado=:es,anu_telefone=:te,anu_codigopostal=:co,anu_disponibilidade=:di,anu_wcprivativo=:wc,anu_mobilada=:mob,anu_utensilios=:ut,anu_internet=:in,anu_rapazes=:rap,anu_raparigas=:ra,anu_despesas=:des,anu_animais=:ani,anu_latitude=:la,anu_longitude=:lo,anu_preco=:pre Where anu_id=:an");
...
//lista todos os anuncios
  function listar_anuncios($opcao){
    ...
    $i=0;
			while($row = $STH->fetch()){
        $arrayAnuncios[$i]=new Anuncio($row->anu_id,$row->anu_titulo,$row->anu_descricao,$row->uti_id,$row->anu_morada,$row->anu_telefone,$row->anu_email,$row->anu_codigopostal,$row->anu_data,$row->anu_disponibilidade,$row->anu_estado,$row->anu_preco,[],$row->anu_wcprivativo,$row->anu_mobilada,$row->anu_utensilios,$row->anu_internet,$row->anu_raparigas,$row->anu_rapazes,$row->anu_despesas,$row->anu_animais,$row->anu_latitude,$row->anu_longitude);
        $i++;
      }
    return $arrayAnuncios;
  }
